...
- =my/bw-api-item=           :: BW item name holding API keys as custom fields
...
(defvar my/bw-api-item "emacs-api-keys"
  "Bitwarden item that stores provider API keys as custom fields.")

(defvar my/api-key-fields
  '("GEMINI_API_KEY" "ANTHROPIC_API_KEY" "OPENAI_API_KEY" "GROQ_API_KEY")
  "API key environment variable names.
The same names are used as custom field names in `my/bw-api-item'.")
...
(defvar my/bw-legacy-api-items
  ...
  "Legacy per-provider Bitwarden item names used before `my/bw-api-item'.
When present, their values are migrated into consolidated custom fields.")
...
available:
  - Bitwarden (preferred) — custom field GitCryptKey on my/bw-github-item
...
Use when you've saved an invalid key and want bootstrap to re-prompt."
  ...
  (let ((cleared-anywhere nil))
    ...
    (when my/bw-session
      ...
      (message "Cleared GitCryptKey in BW '%s'." my/bw-github-item)
...
(defun my/bw-api-key-set (env-var value)
  ...
  (let ((fields `((,env-var . ,value))))
    (if (my/bw-get-item my/bw-api-item)
        (my/bw-update-item my/bw-api-item :fields fields)
      (my/bw-create-item my/bw-api-item
                         :username "emacs"
                         :password ""
                         :fields fields)))
  (let* ((item (my/bw-get-item my/bw-api-item))
    ...
    (unless (and saved (string= saved value))
      (error "BW verification failed: %s was not saved to item %s"
             env-var my/bw-api-item))))

(defun my/api-keys-load ()
  "Populate provider API key env vars.
Order of resolution: one BW item custom field (preferred) → prompt +
persist there when BW is unlocked → Keychain fallback only when BW is
unavailable.  The BW item is `my/bw-api-item'; custom field names are the env
var names in `my/api-key-fields'."
  (let ((api-item (and my/bw-session (my/bw-get-item my/bw-api-item)))
        loaded prompted skipped)
    (dolist (env-var my/api-key-fields)
      (let* ((kc-svc (concat my/api-keychain-service-prefix env-var))
             (value nil)
             (source nil))
        ...
        (when (and my/bw-session (my/-blank-p value))
          (let* ((legacy-name (cdr (assoc env-var my/bw-legacy-api-items)))
            ...
            (unless (my/-blank-p v)
              ...
              (setq api-item (my/bw-get-item my/bw-api-item))
              (setq value v source 'bw-legacy))))
        ;; 3. Keychain fallback only when Bitwarden is unavailable.
        (when (and (not my/bw-session) (my/-blank-p value))
    ...
    (when prompted
      (message "Bootstrap: API keys created via prompt + stored to %s: %s"
               (if my/bw-session
                   (format "Bitwarden item '%s'" my/bw-api-item)
                 "macOS Keychain")
