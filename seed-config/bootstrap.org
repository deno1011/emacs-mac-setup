...
Resolution order, per `my/api-key-fields' entry:
  ...
  2. `my/keychain-service' / account = env-var (when BW is unavailable).
  3. Prompt + persist (BW if unlocked, else Keychain).
  ...
  (let ((api-item (and my/bw-session (my/bw-get-item my/bw-credentials-item)))
    ...
    (dolist (env-var my/api-key-fields)
      (let* ((value nil)
        ...
        ;; 2. Keychain fallback only when Bitwarden is unavailable.
        ...
        ;; 3. Prompt + persist (only when nothing found and not opted out).
