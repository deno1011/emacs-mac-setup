...
Bootstraps the [[https://github.com/progfolio/elpaca][elpaca]] package manager and
=elpaca-use-package= so subsequent modules can use the standard
=(use-package … :ensure …)= recipe form.

This used to live in =~/.emacs.d/init.el='s section 3 — but nothing
in init.el itself needed elpaca, and the FIRST module that actually
calls =use-package= at load time is =20-core.org=. Putting the
bootstrap here keeps init.el thin (data-dir + config-dir + secrets
+ entry point) and groups the heavy "ensure the toolchain is ready"
work with the rest of bootstrap.

Module load order (=config.org='s discovery loop) is
=00= → =05= → =10= → =20= → …. =00-startfirst= and =05-tasks= make
no load-time =use-package= calls, so elpaca being ready by the end
of THIS module's tangle is sufficient. =(elpaca-wait)= at the
bottom of the block blocks until =elpaca-use-package= is built,
guaranteeing that ordering.

If the user clones elpaca for the very first time the
=*elpaca-bootstrap*= buffer appears here instead of during
init.el — the modus-vivendi theme from =00-startfirst= has
already painted by then, so the visual jolt is smaller.

#+begin_src emacs-lisp :tangle yes
;; Forward-declare elpaca's runtime symbols so the byte-compiler
;; doesn't whine about them — elpaca isn't installed at compile time;
;; these all resolve via the autoloads that `elpaca-generate-autoloads'
;; writes during the bootstrap below.
...
;; `elpaca-use-package' here is the elpaca RECIPE name passed as a
;; bare symbol to the `(elpaca elpaca-use-package …)' form below;
;; `use-package-always-ensure' is a use-package customization the
;; package binds after it activates. Forward-declaring both as
;; special variables silences spurious free-variable warnings.
