...
See the section banner for the full design rationale."
  (let ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory)))
    (cond
     ...
     ;; data-dir.el missing AND bw CLI present → ask BW.
     ((my/bw-installed-p)
      (message "Bootstrap: resolving data folder from Bitwarden…")
      (cond
       ((my/bw-unlock)
        (let* ((id (my/github-load-identity))
               (repo (and id (plist-get id :repo)))
          ...
          (cond
           (target
            ...
            (message "Bootstrap: data folder set to %s (from BW.Repo='%s')."
                     target repo))
           (t
            ;; User declined or left repo blank. Per the agreed
            ;; behaviour we fall back to init.el's default; we do NOT
            ;; write data-dir.el so a later launch (with BW.Repo
            ;; filled in) re-runs this resolve.
            (message "Bootstrap: data folder = %s (BW.Repo blank or declined; install.sh fallback)."
                     my/data-dir)))))
       (t
        (message "Bootstrap: data folder = %s (BW skipped or unreachable; install.sh fallback)."
                 my/data-dir))))
...
Bound to the [Save] button. Requires BW unlocked; if not, runs
`my/bw-unlock' synchronously first."
  (interactive)
  (let ((changes (my/-config-form-collect)))
    (cond
     ...
     (t
      ;; Ensure BW is unlocked before any dual-write.
      (unless my/bw-session
        (unless (my/bw-unlock)
