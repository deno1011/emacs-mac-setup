...
(defun my/bootstrap--first-launch-blocking-form (_data-dir-el)
    ...
    (let* ((id   (and my/bw-session
      ...
      (cond
       (target
        ...
        (my/bootstrap-write-data-dir-selection target)
        (message "Bootstrap: data folder set to %s (from BW.Repo='%s'). RESTART Emacs to load config with the new data folder."
                 target repo))
       (t
        (message "Bootstrap: Repo field blank — data-dir.el NOT written. Restart and fill in the Data repo name field to continue.")))))
   (t
...
CURRENT  current stored value (string or nil). Used only to print
         a status indicator on a line ABOVE the input — never
         echoed back into the editable field.
...
HELP     optional one-line help string printed under the input."
  ...
  ;; create code, which crashed the whole form render. Using `?*'
  ;; as the masking char rather than `?●' so it renders consistently
  ;; even on terminal Emacs / minimal fonts.
  (let* ((args (append
        ...
        (let ((changes (my/-config-form-collect))
          ...
          (cond
           ...
           (t
            ...
            ;; BEFORE unlock, so `my/bw-unlock' takes the cached path.
            ...
            (let* ((bw-email-row  (alist-get 'bw-email  changes))
              ...
              (when (and new-master (not (string-empty-p new-master)))
                (my/keychain-set my/keychain-service new-master
                                 my/keychain-account-bw-master)))
            ;; Step 2: ensure BW is unlocked. Uses Keychain creds we
            ;; just wrote in step 1.
            (unless my/bw-session
              (message "Setup form: unlocking Bitwarden vault (this may take 1–2 seconds)…")
              (redisplay t)
              (unless (my/bw-unlock)
                (error "BW unlock declined or failed (master password? bw CLI installed?)")))
            ;; Step 3a: API key rows — dual-write each.
            (dolist (kv changes)
              (let* ((key (car kv))
                     ...
                     (env-key (and (string-prefix-p "api/" kn)
                                   (substring kn 4))))
                (when (and env-key (not (eq val :unchanged)))
                  (message "Setup form: writing %s to BW + Keychain…" env-key)
                  (redisplay t)
            ...
            ;; Step 3b: GH identity rows — accumulate, write as one
            ;; update/create.
            (let ((gh-fields '()) gh-user gh-token any-gh)
              (dolist (kv changes)
                (let ((key (car kv))
                  ...
                  (when (and (not (eq val :unchanged))
                             (not (eq val :skip))
                             (memq key '(gh-username gh-token gh-email
                                         gh-fullname gh-repo)))
                    (setq any-gh t)
                    (pcase key
              ...
              (when any-gh
                (message "Setup form: writing GitHub identity bundle to BW…")
                (redisplay t)
                (let ((item (my/bw-get-item my/bw-credentials-item)))
                  (cond
                   (item
