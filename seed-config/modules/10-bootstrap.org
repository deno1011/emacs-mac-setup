...
step returns a symbol:
  ...
  =:restart-needed=  — step ran the user-facing portion successfully but
                       config has shifted in a way that requires an Emacs
                       restart (e.g. data-dir.el was just written)
...
job is sequencing + reacting to `:restart-needed' (data-dir.el was
just written; stop so the user can restart) or `:error' (display a
warning, stop)."
  (interactive)
  ...
  (catch 'my/bootstrap-bail
    ...
    ;;    is already settled (silent silent restore of data-dir.el
    ;;    from probe item happens here too).
...
:restart-needed  → step already messaged the user; throw `my/bootstrap-bail'
(:error REASON)  → warn with REASON, throw `my/bootstrap-bail'."
  (pcase step-result
    (:done nil)
    (:skip nil)
    (:restart-needed
     (throw 'my/bootstrap-bail :restart-needed))
    (`(:error ,reason)
     (display-warning 'emacs-setup
...
Reads `my/-bootstrap-probe-data' (populated by step 1). When
`data-dir.el' is missing but the probe captured a BW `Repo' field,
writes data-dir.el silently here — no extra BW round-trip.

Outcome:
  :skip            — every required credential is settled; form not opened.
  :done            — form opened, user saved, data-dir.el was already on
                     disk (subsequent-launch path); bootstrap continues.
  :restart-needed  — form opened, user saved, data-dir.el was JUST written
                     (first-launch path); modules tangled with init.el's
                     default need a restart before the remaining steps run.
  (:error REASON)  — form cancelled or saved-but-incomplete."
  ...
  (let* ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory))
         (existed?    (file-exists-p data-dir-el)))
    ;; Silent restore: if data-dir.el is missing but probe captured a
    ;; Repo field, write data-dir.el now using the existing session.
    (unless existed?
      (let* ((item   (plist-get my/-bootstrap-probe-data :item))
             (repo   (and item (my/bw-item-field item "Repo")))
             (target (and (not (my/-blank-p repo))
                          (my/bootstrap-data-dir-for-repo repo))))
        (when target
          (setq my/data-dir target)
          (my/bootstrap-write-data-dir-selection target)
          (message "Bootstrap step 2: data folder = %s (restored from probe BW.Repo='%s')."
                   target repo))))
    (cond
     ...
     (t
      ...
      (recursive-edit)
      (let ((now? (file-exists-p data-dir-el)))
        (cond
         ;; First-launch path: data-dir.el was just written.
         ((and (not existed?) now?)
          (message "Bootstrap: data folder set to %s. RESTART Emacs to load config with the new data folder."
                   my/data-dir)
          :restart-needed)
         ;; Form was saved but data-dir.el still missing: Repo blank.
         ((and (not now?) (eq my/-config-form-result 'saved))
          '(:error "form saved but data-dir.el not written — fill the 'Data repo name' field and restart"))
         ;; Cancelled or otherwise unsubmitted with data-dir.el missing.
         ((not now?)
          '(:error "setup form cancelled — bootstrap halted; restart to re-open"))
         ;; Subsequent-launch path: form recorded changes; continue.
         (t
          :done)))))))
