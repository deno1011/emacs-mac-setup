...
;; sees it in `my/bootstrap-step--open-config-form-and-restart' (Step 13)
...
The next opening of `my/bootstrap-config-form' will show the row as \"[not set]\" so it can be re-entered."
  (interactive
...
Called from =config.org='s =emacs-startup-hook= AFTER all modules have
loaded and the first frame is painted. The orchestrator
(=my/bootstrap-orchestrate=, Step 13) owns the bootstrap recipe; this
entry point fires the two fire-and-forget background tasks (distro
config refresh, daemon-service install) and then runs the
...
\(e.g. `my/bootstrap-step--open-config-form-and-restart') to tell which
...
`my/bootstrap-step--open-config-form-and-restart' can fall through to their
no-input fallback."
  (interactive)
...
;; — fired from `emacs-startup-hook' via `my/bootstrap' — now owns the
;; entire flow including data-dir resolution. Modules that depend on
;; `my/data-dir' tangle with init.el's default; the first launch shows
;; the setup form, Save writes data-dir.el, and the second launch
;; tangles modules with the correct value.)
#+end_src
...
restart) or `:error' (display a warning, stop)."
  ...
  (catch 'my/bootstrap-bail
    ...
    ;; 2. Where does this Mac's data live?
...
Keys:
  ...
  :data-dir-el-exists?  t when ~/.emacs.d/data-dir.el is on disk.
...
(defun my/bootstrap-step--probe-credentials ()
  "Silent probe of cached credentials.

Reads `my/keychain-account-bw-email' + `my/keychain-account-bw-master'
from Keychain. If both present, calls `my/bw--unlock-internal' to
obtain a session, then reads `my/bw-credentials-item' via
`my/bw-get-item'. Stashes the harvest in `my/-bootstrap-probe-data'
for the next step (`resolve-data-dir' in this commit;
`ensure-form-submitted' once the form gate moves here) and the form
prefill.

Always returns :done — failure to unlock or fetch the item leaves
`:item' nil, never raises. On success, sets `my/bw-session' so
subsequent steps reuse the same unlock."
  ...
  (let* ((email   (my/keychain-get my/keychain-service
    ...
    (setq my/-bootstrap-probe-data
          (list :bw-master  master
                :bw-email   email
                :bw-unlocked? (and session t)
                :item       item
                :data-dir-el-exists?
                (file-exists-p
                 (expand-file-name "data-dir.el" user-emacs-directory))))
    (message "Bootstrap step 1: probe — BW %s, item %s, data-dir.el %s."
             (if session "unlocked" "not unlocked")
             (if item    "found"    "missing")
             (if (plist-get my/-bootstrap-probe-data :data-dir-el-exists?)
                 "present" "absent")))
  :done)
...
Returns t when the form can be skipped; nil otherwise."
  (let* ((probe   my/-bootstrap-probe-data)
    ...
    (and (funcall set-p (plist-get probe :bw-master))
         ...
         (or (plist-get probe :data-dir-el-exists?)
             (funcall set-p (funcall bw-f "Repo")))
         (or gh-out
...
Returns :done / :skip / (:error …)."
  ...
  (let ((id (my/github-load-identity)))
    ;; Safety net: Step 1's fast path (data-dir.el on disk) returns
    ;; :done without unlocking BW, so `id' may be nil here purely
    ;; because `my/bw-session' is unset. If the data folder is in any
    ;; way unhealthy we MUST validate against the real identity —
    ;; force a silent BW unlock (Keychain-cached creds, no prompts on
    ;; the happy path) and retry. If identity is still nil after the
    ;; unlock attempt, we have no way to validate or clone → abort.
    (when (and (not id)
...
Invoked by the orchestrator's `:need-input' dispatch path. The form
itself dual-writes BW + Keychain on Save AND writes data-dir.el (see
the form save handler in Step 12). All we do here is open the form,
wait, and emit the correct restart message based on what landed."
  (let ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory)))
    (message "Bootstrap: opening setup form (data-dir.el not yet written; bootstrap paused)…")
