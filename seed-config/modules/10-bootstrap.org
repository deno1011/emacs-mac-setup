...
the pre-clear set: every refresh wipes these entries first so that
a BW-side rename/deletion produces an empty Keychain (predicate sees
\"needs work\") instead of leaving stale values masquerading as fresh
ones.

Includes: github metadata accounts + every `my/api-key-fields' entry.

Excludes: bw-email + bw-master (the BW unlock cache — clearing those
would force the user to re-enter the master password every launch),
and the entire git-crypt service (those keys are per-repo and a user
may legitimately keep keys for repos not named in the current BW
credentials item)."
  (append
...
may be nil (defaults to `user-login-name'). ON-FINISH is invoked on
the main thread with t (always — individual missing entries are NOT
errors here, just no-ops in `security').

Returns the task name (symbol) so callers can wire `:depends-on'.
Used by the BW->Keychain refresh to wipe stale GitHub + API values
BEFORE the bw chain runs — if BW now reports the credentials item
as missing, Keychain stays empty and the predicate downstream sees
\"needs work\" instead of trusting a stale cache."
    ...
    (cond
     ...
     ((not (fboundp 'my/task-shell))
      ...
      (dolist (sa svc-acc-pairs)
        (my/keychain-delete (car sa) (cdr sa)))
      (funcall cb t)
      nil)
     (t
      (let* ((task-name (intern (format "kc-multi-delete-%d"
             ...
             (script
              (concat
               (mapconcat
                (lambda (sa)
                  (format "security delete-generic-password -a %s -s %s >/dev/null 2>&1 || true"
                ...
                (let* ((s (and (stringp out) (string-trim out)))
                  ...
                  (cond
                   (session
                    ...
                    ;; BW_SESSION env var, NOT argv. CRITICAL: pass a
                    ;; `copy-sequence' of the session string. The task
                    ;; framework calls `clear-string' on every :env value
                    ;; after `make-process' (secret hygiene, see
                    ;; 05-tasks.org's `clear-string' on (cdr kv)). If we
                    ;; passed the original `session', that mutation would
                    ;; also zero `my/bw-session' (same string object),
                    ;; making every subsequent bw call fail with
                    ;; "Vault is locked".
                    (my/task-shell
