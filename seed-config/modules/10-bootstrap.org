...
Preserved (deliberately):
  ...
  (append
   ...
   ;; Sourced live from BW.emacs-credentials' GitCryptKey:<repo> fields
                ...
                (let* ((s (and (stringp out) (string-trim out)))
                  ...
                  (cond
                   (session
                    ...
                    ;; user renaming `emacs-credentials' server-side
...
IMPORTANT — exact-name check on `direct': the bw CLI's `bw get item NAME'
does SUBSTRING matching, not exact. If the vault contains
`old_emacs-credentials' but not `emacs-credentials', then `bw get
item emacs-credentials' will substring-match and return
`old_emacs-credentials' silently. Without the `string=' guard below,
this function would return the wrong item and downstream code would
treat it as if the originally-named entry still existed. The check
forces us into the duplicate-fallback path, which also exact-name
filters and so correctly returns nil for the rename case."
  (when my/bw-session
    (let* ((out (with-output-to-string
                  (with-current-buffer standard-output
                    (call-process-shell-command
      ...
      (or (and direct-name-matches direct)
          ;; Either direct returned a substring match (reject) or duplicate
          ;; item names made `bw get item NAME' ambiguous. Fall back to an
          ;; exact-name list search.
          (let* ((list-out (with-output-to-string
                             (with-current-buffer standard-output
                               (call-process-shell-command
                                (format "bw --nointeraction list items --search %s --session %s 2>/dev/null"
                                        (shell-quote-argument item-name)
                                        (shell-quote-argument my/bw-session))
                                nil t))))
       ...
       (lambda (out)
         (let* ((parsed
           ...
           (cond
            ...
            ;; on substrings. If the user renames `emacs-credentials'
            ;; to `old_emacs-credentials' server-side, bw still returns
...
;; Explanation: handle async bw failures, with duplicate-item recovery.
(defun my/-bw-get-item-async-handle-failure (cb item-name err)
  "Map a raw bw-cli stderr ERR into either a recovered item or a
clear user-facing error string. Calls (funcall CB ITEM ERR-OR-NIL).

Currently special-cases `More than one result was found' (duplicate
items in BW vault — happens when bw cli's name search is fuzzy
enough to match multiple items, e.g. `emacs-credentials' fuzzy-
matches both `emacs_credentials' and `old_emacs_credentials').
Falls back to the SYNC `my/bw-get-item' on that path, which already
implements the duplicate fallback via `bw list items --search NAME'
+ exact-name filter + most-fields sort. Other errors pass through
unchanged as a callback error string."
  (let ((msg (or err "bw exited non-zero")))
    (cond
     ((and (stringp msg)
           (string-match-p "More than one result was found" msg))
      ;; Recovery: sync version's duplicate fallback picks the exact-
      ;; name match with the most fields. Synchronous but only fires
      ;; in the rare duplicate-error case (already on the main thread
      ;; in the callback context).
      (let ((item (my/bw-get-item item-name)))
        (cond
         (item (funcall cb item nil))
         (t    (funcall cb nil
                        (format "BW vault has multiple items fuzzy-matching '%s' \
and none with that EXACT name. Rename the desired item to '%s' in the BW \
web/desktop client and restart Emacs. bw said: %s"
...
After Save: BW is unlocked + emacs-credentials populated. Read the
...
Sets `my/data-dir' to the BW.Repo-derived value and writes
`~/.emacs.d/data-dir.el' so subsequent launches fast-path. Idempotent:
calling again after success is a no-op via the data-dir.el branch.

See the section banner for the full design rationale."
  (let ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory)))
    (cond
     ;; Fast path: data-dir.el already on disk (written by a previous
     ;; install.sh from $EMACS_DATA_DIR, by a previous run of this
     ;; function, or by `my/bootstrap-ensure-selected-repo-dir' on a
     ;; repo-switch launch). `init.el' already loaded it earlier in
     ;; this Emacs run, so `my/data-dir' is already correct — nothing
     ;; to do.
     ((file-exists-p data-dir-el)
      (message "Bootstrap: data folder = %s (cached in %s)."
               my/data-dir data-dir-el))
     ;; data-dir.el missing → CANNOT show the form here. We're inside
     ;; `init.el' execution; the initial frame isn't on screen yet, so
     ;; `pop-to-buffer' + `recursive-edit' would block the event loop
     ;; with no displayed window (frozen Emacs). The form invocation
     ;; was moved to `my/bootstrap' which runs on `emacs-startup-hook'
     ;; — after the frame is visible. Here we just message and bail.
     ;; Module-level defensive gates (`(when (file-exists-p data-dir.el)
     ;; …)' in 50-org.org / 55-apple-reminders.org / 75-wiki.org)
     ;; ensure no module writes anything under the default `~/emacs/'
     ;; while we're waiting for the user to fill in the form.
     (t
      (message "Bootstrap: data-dir.el missing — setup form will open after Emacs is ready (waiting for frame).")))))
...
fields are missing."
  ...
  (let* ((buf (get-buffer-create "*Emacs Setup*"))
    ...
    (widget-insert "cache) AND your Bitwarden 'emacs-credentials' item (cross-Mac source).\n")
    ...
    (widget-insert "\nGitHub identity (in BW.emacs-credentials, also cached in Keychain)\n")
    (widget-insert     "----------------------------------------------------------------\n")
