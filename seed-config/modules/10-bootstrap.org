...
;; Explanation: Start the BW-to-Keychain refresh as a visible task.
(defun my/bootstrap-start-keychain-refresh-task (&optional after-refresh)
  "Submit the startup Bitwarden-to-Keychain refresh task.
Uses the task framework from 05-tasks when available. The refresh runs
at the beginning of bootstrap, before the main bootstrap task starts.
AFTER-REFRESH is called after success or failure so startup can continue
from the existing Keychain cache if BW is unavailable."
  ...
  (cond
   ((fboundp 'my/task-elisp)
    (my/task-elisp
     'bw-keychain-refresh
     #'my/keychain-refresh-from-bw--sync
     :on-success
     (lambda (summary)
       (setq my/bootstrap-keychain-refresh-failed nil)
       ;; Stash the async flush task name so the next bootstrap stage
       ;; can declare :depends-on. nil means writes happened sync or
       ;; entries were empty — nothing to wait on.
   ...
   (t
	...
	(let ((summary (my/keychain-refresh-from-bw--sync)))
          (setq my/bootstrap-keychain-refresh-failed nil)
	  (message "BW->Keychain startup refresh: GitHub=%d, API=%d, git-crypt=%d"
	           (or (plist-get summary :github) 0)
	           (or (plist-get summary :api) 0)
                   (or (plist-get summary :gitcrypt) 0)))
      (error
       (setq my/bootstrap-keychain-refresh-failed t)
       (display-warning 'emacs-setup
                        (format "BW->Keychain startup refresh failed: %s"
                                (error-message-string err))
                        :warning)))
    (my/bootstrap-after-keychain-refresh))))
