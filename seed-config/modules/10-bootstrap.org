...
Returns a plist with summary counts. This is intended to run as a task
at the beginning of bootstrap."
  (unless (my/bw-unlock)
          ...
          (if (and skipped (string= skipped my/api-skipped-sentinel))
            ...
            (dolist (pair `((,my/keychain-account-github-username . ,username)
              ...
              (unless (my/-blank-p (cdr pair))
                (my/keychain-set my/keychain-service (cdr pair) (car pair))
        ...
        (dolist (field (my/bw-item-fields-list item))
          (let ((name (cdr (assoc 'name field)))
            ...
            (when (and (stringp name)
              ...
              (my/keychain-set my/git-crypt-keychain-service
                               value
                               (match-string 1 name))
              (my/keychain-set my/git-crypt-keychain-service value name)
              (cl-incf gitcrypt-count))))
        (list :github github-count :api api-count :gitcrypt gitcrypt-count))
    (my/bw-lock)))
