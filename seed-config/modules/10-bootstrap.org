...
First-launch gate: if `~/.emacs.d/data-dir.el' is missing, this
function REFUSES to proceed past the form. It opens
`my/bootstrap-config-form' synchronously, blocks on `recursive-edit',
and on Save writes data-dir.el + tells the user to restart. Cancel
or any other exit also bails out (the form re-opens on the next
restart since data-dir.el still doesn't exist). NO other bootstrap
work (keychain refresh, repo clone, gh auth, api-keys-load) runs
until the user has chosen a data folder."
  (interactive)
  (let ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory)))
    ;; Diagnostic: log the decision path explicitly so a misfiring form
    ;; gate becomes debuggable without filesystem access. Paste this
    ;; from *Messages* if the form opens unexpectedly.
    (message "Bootstrap: gate check — data-dir.el=%s exists=%s my/data-dir=%s user-emacs-directory=%s"
             data-dir-el
             (if (file-exists-p data-dir-el) "YES" "NO")
             (if (boundp 'my/data-dir) my/data-dir "<unbound>")
             user-emacs-directory)
    (cond
     ((not (file-exists-p data-dir-el))
      ;; First-launch path: form is mandatory.
      (my/bootstrap--first-launch-blocking-form data-dir-el))
     (t
      ;; Steady-state path: data-dir.el present, proceed normally.
...
Returns :done / :skip / (:error REASON)."
  ...
  ;; STUB — wired in commit 3. For now, delegate to existing logic
  ;; via `my/bootstrap--run-full-sync' inside `my/bootstrap-orchestrate-legacy'.
  :skip)

(defun my/bootstrap-step--unlock-git-crypt ()
  "If the cloned repo uses git-crypt, unlock with the key from BW.
Returns :done / :skip / (:error REASON)."
  (message "Bootstrap step 3: git-crypt unlock (no-op if not encrypted)…")
  :skip)

(defun my/bootstrap-step--refresh-keychain ()
  "Pre-clear stale Keychain entries; rewrite from BW.
...
(defun my/bootstrap-step--authenticate-gh ()
  "Authenticate the gh CLI using the BW-stored token if not already.
Returns :done / :skip / (:error REASON)."
  (message "Bootstrap step 6: authenticate gh CLI…")
  :skip)

(defun my/bootstrap-step--finalize ()
  "Lock the BW vault, emit a one-line summary. Returns :done."
