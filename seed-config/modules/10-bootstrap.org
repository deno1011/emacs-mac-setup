    ...
    (cond
     ...
     (t
      (my/task-shell
       ...
       :on-failure
       (lambda (err)
         (funcall cb nil (my/-bw-get-item-async-format-error item-name err))))))
    nil))

;; Explanation: format bw async errors, recognising duplicate-item case.
(defun my/-bw-get-item-async-format-error (item-name err)
  "Map a raw bw-cli stderr ERR into a clearer user-facing error string.
Currently special-cases `More than one result was found' (duplicate
items in BW vault) with a remediation hint. Other errors pass
through unchanged."
  (let ((msg (or err "bw exited non-zero")))
    (cond
     ((and (stringp msg)
           (string-match-p "More than one result was found" msg))
      (format "BW vault has DUPLICATE items named '%s'. \
Delete the stale one in the BW web/desktop client and restart Emacs. \
bw said: %s"
              item-name (string-trim msg)))
     (t msg))))
