      ...
      (cond
       ...
       (t
        ...
        (let ((default-directory dir)
          ...
          ;; never reaches the GitHub remote. No-op if git-crypt can't be
          ;; installed; user data falls back to plaintext gracefully.
          (my/-repo-auto-init-git-crypt dir)
          (call-process-shell-command
        ...
        (with-current-buffer (get-buffer-create "*repo-pull*")
          (when (string-match-p "CONFLICT\\|Could not apply" (buffer-string))
            (display-warning 'emacs-setup
                             ...
                             :warning)))))
     ;; Empty / absent / only-seed-config → clone or create.
     ;; The "only-seed-config" case fires right after a fresh install.sh
     ;; rsync, when `config/' is the sole top-level entry. That isn't user
     ;; data; it's just the installer's seed. Treat as effectively empty so
     ;; my/-repo-fetch-or-create can go through the create branch and fire
     ;; auto-init (no adoption prompt).
     ((let ((entries (and (file-directory-p dir)
                          (directory-files dir nil "\\`[^.]"))))
        (or (not (file-exists-p dir))
            (null entries)
            (equal entries '("config"))))
      (my/-repo-fetch-or-create identity))
     ;; Has user-content but wrong/no remote — adopt local files into the
...
(defun my/git-crypt-key-prompt-and-save ()
  "Prompt for a base64-encoded git-crypt key with hidden input.
Validate before saving. On success, save the NORMALIZED form to BOTH
Bitwarden (custom field GitCryptKey on `my/bw-credentials-item') AND
macOS Keychain (service `my/git-crypt-keychain-service'). Returns the
normalized key or nil.

Empty input persists the `__SKIPPED__' sentinel so future bootstraps
do NOT re-prompt. Run `M-x my/git-crypt-reset' to revive prompting.
...
with:  `git-crypt export-key /tmp/k && base64 -i /tmp/k | pbcopy'"
  (let* ((entered (read-passwd
                   "Base64-encoded git-crypt key (empty = skip permanently, won't re-ask): "))
         (normalized (and entered (my/-normalize-base64 entered))))
    (cond
     ((my/-blank-p entered)
      ;; Empty = explicit opt-out. Persist sentinel in BOTH stores so
      ;; future launches skip the prompt entirely.
      (when my/bw-session
        (ignore-errors
          (my/bw-update-item my/bw-credentials-item
                             :fields `(("GitCryptKey" . ,my/api-skipped-sentinel)))))
      (my/keychain-set my/git-crypt-keychain-service my/api-skipped-sentinel)
      (display-warning
       'emacs-setup
       "git-crypt key skipped — future launches will not prompt.
M-x my/git-crypt-reset clears the sentinel and re-prompts."
       :warning)
      nil)
...
then paste again (M-x my/git-crypt-step). NOT saved; next launch will re-prompt."
       :warning)
      nil)
     (t
      ;; Dual-write: BW (canonical) AND Keychain (so future fallback works).
      (when my/bw-session
        (ignore-errors
          (my/bw-update-item my/bw-credentials-item
...
;; Explanation: User command — clear sentinel/key so next bootstrap re-prompts.
(defun my/git-crypt-reset ()
  "Clear the git-crypt key (or skip sentinel) from BOTH BW and Keychain.
Next `my/bootstrap' will re-prompt. Use after rotating the key or to
revive prompting after a previous opt-out."
  (interactive)
  (when (my/bw-unlock)
    (unwind-protect
  ...
  (message "git-crypt key cleared from BW + Keychain. Next bootstrap will prompt."))
...
Order:
  1. IDENTITY plist's :git-crypt-key (= BW's GitCryptKey field)
  2. macOS Keychain (service my/git-crypt-keychain-service)
  3. Prompt the user and persist

If any source returns `my/api-skipped-sentinel', the user has opted
out — return nil with no prompt. Run `M-x my/git-crypt-reset' to
revive prompting."
  (let ((bw-val (plist-get identity :git-crypt-key))
        (kc-val (my/keychain-get my/git-crypt-keychain-service)))
    (cond
     ;; Opt-out sentinel in either store → never prompt.
     ((or (and (stringp bw-val) (string= bw-val my/api-skipped-sentinel))
          (and (stringp kc-val) (string= kc-val my/api-skipped-sentinel)))
      (message "Bootstrap: git-crypt skipped by user (M-x my/git-crypt-reset to revive).")
      nil)
     ;; 1. From BW (already in identity)
     ((not (my/-blank-p bw-val))
      (message "Bootstrap: git-crypt key from BW '%s' (GitCryptKey)."
               my/bw-credentials-item)
      bw-val)
     ;; 2. macOS Keychain fallback
     ((not (my/-blank-p kc-val))
      (message "Bootstrap: git-crypt key from macOS Keychain (service %s)."
               my/git-crypt-keychain-service)
      kc-val)
     ;; 3. Prompt + persist
     (t
...
;; (Old `my/git-crypt-clear-key' superseded by `my/git-crypt-reset' above.)
...
(defun my/-repo-auto-init-git-crypt (dir)
  "Initialize git-crypt inside DIR (a fresh `git init'd directory),
write `.gitattributes' patterns from `my/git-crypt-default-attributes',
export the generated key, and save the base64-encoded key to BOTH
Bitwarden (custom field GitCryptKey on `my/bw-credentials-item') AND
macOS Keychain (service `my/git-crypt-keychain-service').

Called from `my/-repo-fetch-or-create' in the new-repo branch
BEFORE the first `git add' so the filter encrypts matching files into
the very first blob — plaintext never reaches the remote.

No-op + warning if git-crypt is not installed and cannot be installed.
Idempotent: skips if `.git/git-crypt/keys/default' already exists."
  (cond
   ((file-exists-p (expand-file-name ".git/git-crypt/keys/default" dir))
                 ...
                 (t
                  (let* ((raw (with-temp-buffer
                    ...
                    (when my/bw-session
                      (ignore-errors
                        (my/bw-update-item my/bw-credentials-item
                                           :fields `(("GitCryptKey" . ,b64)))
                        (message "Bootstrap: git-crypt key saved to BW '%s' (GitCryptKey field)."
                                 my/bw-credentials-item)))
                    (my/keychain-set my/git-crypt-keychain-service b64)
                    (message "Bootstrap: git-crypt initialized in %s; key saved to Keychain (service %s)."
                             dir my/git-crypt-keychain-service))))
...
content. Plan accordingly."
  ...
  (cond
   ...
   (t
    ...
    (my/-repo-auto-init-git-crypt my/data-dir)
    (when my/bw-session (my/bw-lock))
    (message "git-crypt-enable-now: done. Commit + push to apply encryption to the next blob."))))

;; Explanation: User command — persist git-crypt skip sentinel WITHOUT prompting.
(defun my/git-crypt-skip ()
  "Persist the `__SKIPPED__' sentinel into BW + Keychain so future
bootstraps skip the git-crypt step entirely. Use when you do NOT
want git-crypt enabled on this repo and want to silence the
auto-init behaviour. Revive with `M-x my/git-crypt-reset'."
  (interactive)
  (when (my/bw-unlock)
    (unwind-protect
        (ignore-errors
      ...
      (my/bw-lock)))
  (my/keychain-set my/git-crypt-keychain-service my/api-skipped-sentinel)
  (message "git-crypt: skip sentinel persisted in BW + Keychain. Bootstrap will not auto-init or prompt. M-x my/git-crypt-reset to revive."))
...
Behaviour when the repo declares git-crypt (=.gitattributes= has
=filter=git-crypt=) and is not already unlocked:

  1. Key found in BW or Keychain (and not the skip sentinel)
     → `my/git-crypt-unlock' uses it.
  2. Skip sentinel set in either store
     → silent no-op. Run `M-x my/git-crypt-reset' to revive.
  3. No key, no sentinel
     → `my/-repo-auto-init-git-crypt' fires AUTOMATICALLY. Generates
       a fresh key, dual-saves to BW + Keychain, encrypts future
       commits. Commits already on the remote stay plaintext (git
       history is immutable; rewrite separately with `git filter-repo'
       if needed).

No prompts. To opt out before bootstrap fires, run
`M-x my/git-crypt-skip' once."
  (cond
    ...
    (cond
     ((not (my/git-crypt-installed-p))
      (display-warning
       ...
       :warning))
     (t
      ;; Read existing stores without side effects.
      (let* ((bw-val (plist-get identity :git-crypt-key))
             (kc-val (my/keychain-get my/git-crypt-keychain-service))
             (opted-out
              (or (and (stringp bw-val)
                       (string= bw-val my/api-skipped-sentinel))
                  (and (stringp kc-val)
                       (string= kc-val my/api-skipped-sentinel))))
             (key
              (or (and (not (my/-blank-p bw-val))
                       (not (string= bw-val my/api-skipped-sentinel))
                       (progn
                         (message "Bootstrap: git-crypt key from BW '%s' (GitCryptKey)."
                                  my/bw-credentials-item)
                         bw-val))
                  (and (not (my/-blank-p kc-val))
                       (not (string= kc-val my/api-skipped-sentinel))
                       (progn
                         (message "Bootstrap: git-crypt key from Keychain (service %s)."
                                  my/git-crypt-keychain-service)
                         kc-val)))))
        (cond
         (opted-out
         ...
         (t
          ;; No key, no opt-out. Auto-initialize this clone so future
          ;; commits encrypt without any user prompt. Existing history
          ;; on the remote stays plaintext — accept that, or opt out
          ;; with M-x my/git-crypt-skip BEFORE the next bootstrap run.
          (message "Bootstrap: no git-crypt key found in BW or Keychain — auto-initializing this clone (no prompts).")
          (my/-repo-auto-init-git-crypt my/data-dir)))))))))
#+end_src
