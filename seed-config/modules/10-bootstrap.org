...
Two phases:
  ...
  with `:bw-master', `:bw-email', `:data-dir-el-exists?', and
  `:item nil' / `:bw-unlocked? nil'. Then run
  `my/bootstrap--probe-data-complete-p' — the predicate already
  consults Keychain for every form field (GH identity, API keys)
  via the helpers it calls. If the answer is t, the probe ends
  here: no `bw unlock' subprocess, no `bw get item' subprocess,
  no BW session opened.
  ...
  `:bw-unlocked?' + `:item'. Failure (BW not installed, master
  rotated, network) is non-fatal — `:item' stays nil and the
  next step (`ensure-form-submitted') opens the form to ask the
  user.
  ...
  (let* ((email  (my/keychain-get my/keychain-service
    ...
    (setq my/-bootstrap-probe-data
          (list :bw-master master
                ...
                :bw-unlocked? nil
                :item nil
                :data-dir-el-exists? dde-p))
    (cond
     ;; Phase 1 was enough — short-circuit, no BW touch.
     ((my/bootstrap--probe-data-complete-p)
      (message "Bootstrap step 1: probe — Keychain complete, BW untouched, data-dir.el %s."
               (if dde-p "present" "absent")))
