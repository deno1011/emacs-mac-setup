      ...
      (let* ((email  (my/keychain-get my/keychain-service
        ...
        (cond
         ((not (and email master))
          ...
          (run-at-time 0 nil cb (my/bw-unlock)))
         (t
          (my/task-shell
           'bw-unlock
...
(defun my/bootstrap--step-identity (&rest _)
  "After BW unlock: load GitHub identity, set git global config,
chain to gh install + auth check.
The `bw get item' inside `my/github-load-identity' is one ~200ms
shell call — kept sync because by now UI is already interactive."
  (let ((id (my/github-load-identity)))
    ...
    (cond
     ...
     (t
      ;; No usable identity — still write starter content locally, then finish.
...
(defun my/bootstrap--step-api-keys (_id)
  "Populate Keychain runtime cache (sync — Keychain reads are fast,
BW pull only happens on first launch when the cache is empty), then
fire the final lock + finish."
  (my/api-keys-load)
  (my/bootstrap--finish))
