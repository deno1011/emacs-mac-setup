  ...
  (let ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory)))
    (cond
     ...
     ((my/bw-installed-p)
      ...
      ;; CRITICAL during early init: the initial frame may exist but not
      ;; be displayed yet (`window-setup-hook' hasn't fired since
      ;; init.el is still running). Without forcing visibility +
      ;; redisplay, `pop-to-buffer' above puts the form in a window the
      ;; user can't see, and `recursive-edit' below blocks the event
      ;; loop indefinitely — Emacs appears frozen on a blank screen.
