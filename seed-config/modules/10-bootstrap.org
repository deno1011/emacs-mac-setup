  ...
  (cond
   ...
   (t
    (let ((task-name (intern (format "kc-multi-set-%d"
      ...
      (my/task-shell
       task-name
       (list "bash" "-c" (my/-keychain-multi-set-script entries))
       ;; The script contains shell-quoted secret VALUES on its argv
       ;; (passed as the `bash -c' string). Redact wipes the task
       ;; buffer + result slot after completion. The script's argv
       ;; itself is still briefly visible to `ps -ef' during execution
       ;; — that's a deeper leak addressed separately (move to stdin).
       :redact t
    ...
    (let ((session my/bw-session))
      ...
      (when (fboundp 'my/task-shell)
        (my/task-shell
         ...
         ;; Argv contains the session token inside the `bash -c'
         ;; string. Stdout is empty (redirected) but redact for
         ;; consistency in case `bw' starts writing diagnostics.
         :redact t)))))
           ...
           (cond
            ...
            (t
             (my/task-shell
              ...
              :env `(("BW_PW" . ,master))
              ;; Stdout IS the BW session token. Redact the buffer +
              ;; result so neither the *task: bw-unlock-N* buffer nor
              ;; `(my/task-result-of 'bw-unlock-N)' holds the secret
              ;; after on-success has stashed it in `my/bw-session'.
              :redact t
              :on-success
              (lambda (out)
                (let* ((s (and (stringp out) (string-trim out)))
                  ...
                  (cond
                   (session
                    (setq my/bw-session session)
                    ;; Background sync — fire-and-forget. Argv contains
                    ;; the session token (via shell-quote-argument inside
                    ;; the bash -c string) so we redact the buffer too.
                    (my/task-shell
    ...
    (cond
     ...
     (t
      (my/task-shell
       ...
       ;; GitCryptKey field. Redact aggressively — the on-success
       ;; callback parses it in-memory and the buffer/result get wiped.
       ;; Argv contains the session token as well; same separate fix
       ;; needed for argv leak.
       :redact t
       :on-success
       (lambda (out)
         (let ((parsed
