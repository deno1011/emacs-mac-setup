...
20-core / 50-org / 55-apple-reminders et al. tangle anything
...
Outcome:
  ...
  :done            — form opened and user saved; `my/data-dir' is now
                     final and bootstrap continues. Since the orchestrator
                     runs synchronously BEFORE later modules tangle (see
                     Step 9), there is no \"restart needed\" branch.
    ...
    (cond
     ...
     (t
      ...
      (let ((now? (file-exists-p data-dir-el)))
        (cond
         ;; Form saved AND data-dir.el is on disk → continue. Since
         ;; the orchestrator runs synchronously before later modules
         ;; tangle, `my/data-dir' is already final for the rest of
         ;; this Emacs session; no restart needed.
         (now? :done)
         ;; Form was saved but data-dir.el still missing: Repo blank.
         ((eq my/-config-form-result 'saved)
          '(:error "form saved but Repo field was blank — re-open the form and fill the 'Data repo name' field"))
