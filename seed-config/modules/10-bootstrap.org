    ...
    (let ((session my/bw-session))
      ...
      (when (fboundp 'my/task-shell)
        (my/task-shell
         ...
         ;; `ps -ef' otherwise exposes). `--nointeraction' makes bw
         ;; exit instead of prompting if the session is stale.
         (list "bw" "--nointeraction" "lock")
                  ...
                  (cond
                   (session
                    ...
                    (my/task-shell
                     ...
                     :env `(("BW_SESSION" . ,(copy-sequence session)))
                     :redact t
    ...
    (cond
     ...
     (t
      (my/task-shell
       ...
       ;; CRITICAL: `copy-sequence' the session — the task framework
       ;; calls `clear-string' on each :env value after `make-process'
       ;; (secret hygiene). Passing `my/bw-session' directly would
       ;; zero the global, locking out every subsequent bw call.
       (list "bw" "--nointeraction" "get" "item" item-name)
       :env `(("BW_SESSION" . ,(copy-sequence my/bw-session)))
       ...
       :on-failure
       (lambda (err)
         (funcall cb nil (or err "bw exited non-zero"))))))
    nil))
  ...
  (let ((item (my/bw-get-item my/bw-credentials-item)))
    ...
    (let* ((username (and item (my/bw-item-username item)))
      ...
      (cond
       ...
       ;;
       ;; Branch the prompt wording on whether the BW item is genuinely
       ;; absent versus present-but-incomplete. The "absent" path can
       ;; happen because the user renamed/deleted the item out-of-band —
       ;; making the prompt say "Create a NEW BW item …" instead of the
       ;; generic "Set up GitHub sync?" prevents the user from
       ;; reflexively answering yes thinking it's about reusing whatever
       ;; was there. Yes here ALWAYS creates a fresh `emacs-credentials'
       ;; via `my/bw-create-item' (line 992); any renamed-away item
       ;; sitting under a different name is left untouched.
       ((and (or (not item)
                 (and need-token need-username need-repo))
             (not (y-or-n-p
                   (if item
                       "Set up GitHub sync? (creates a private repo to sync your Emacs data across Macs) "
                     (format "BW item '%s' not found — create a NEW one (you'll be prompted for token, email, repo)? "
                             my/bw-credentials-item)))))
...
See the section banner for the full design rationale."
  (let ((data-dir-el (expand-file-name "data-dir.el" user-emacs-directory)))
    (cond
     ...
     ;; data-dir.el missing AND bw CLI present → open the setup form
     ;; (blocking) so the user enters BW creds + GH identity + API keys
     ;; in one pass, then read the just-written `Repo' field to set
     ;; `my/data-dir'. The form's Save handler does all the BW unlock
     ;; + dual-write work; we just consume the result here.
     ((my/bw-installed-p)
      (message "Bootstrap: opening setup form (data-dir.el not yet written)…")
      (setq my/-config-form-result nil)
      (my/bootstrap-config-form)
      (recursive-edit)  ; blocks until Save / Cancel
      (cond
       ((eq my/-config-form-result 'saved)
        ;; Form succeeded: BW is now unlocked + emacs-credentials is
        ;; populated with whatever the user typed. Read the Repo field
        ;; and derive `my/data-dir'.
        (let* ((id   (and my/bw-session
                          (my/bw-get-item my/bw-credentials-item)))
               (repo (and id (my/bw-item-field id "Repo")))
          ...
          (cond
           (target
            ...
            (my/bootstrap-write-data-dir-selection target)
            (message "Bootstrap: data folder set to %s (from BW.Repo='%s')."
                     target repo))
           (t
            ;; User left repo blank on the form — per the agreed
            ;; behaviour we fall back to init.el's default; we do NOT
            ;; write data-dir.el so a later launch re-opens the form.
            (message "Bootstrap: data folder = %s (BW.Repo blank; install.sh fallback)."
                     my/data-dir)))))
       (t
        ;; Cancelled or empty save — same blank-input fallback.
        (message "Bootstrap: data folder = %s (setup cancelled or no input; install.sh fallback)."
                 my/data-dir))))
     ;; bw not installed at all → can't resolve from BW this launch.
     (t
      (display-warning
       'emacs-setup
       (format "Bootstrap: bw CLI not installed; data folder stays at %s. \
Install bw (brew install bitwarden-cli) and restart Emacs to switch."
               my/data-dir)
       :warning)))))

;; SIDE EFFECT at module-load time: resolve `my/data-dir' NOW so that
;; every module loading after this one (20-core, 50-org,
;; 55-apple-reminders, 75-wiki, 80-gtd — all numbered above 10) sees
;; the correctly-derived value and captures the right derived paths.
;; The post-startup `my/bootstrap' on `emacs-startup-hook' still runs
;; for the rest of the work (repo clone, gh auth, api keys, etc.).
...
(defvar my/-config-form-result nil
  "Set by `my/-config-form-save' (`saved') or `-cancel' (`cancelled')
before exiting `recursive-edit'. Used by callers wrapping the form in
a blocking flow (e.g. `my/bootstrap-resolve-data-dir-now') to tell
...
(defun my/-config-form-current-gh-field (item field)
  "Read FIELD ('username 'password 'Email 'Fullname 'Repo) from
ITEM (parsed alist or nil). Returns string or nil."
  (when item
    (cond
...
(defun my/-config-form-insert-row (key label current-value &optional secret skip-eligible)
  "Insert one labelled input row.
KEY is the symbol used in `my/-config-form-rows' (alist key).
LABEL is the displayed label.
CURRENT-VALUE is the currently-stored value (string or nil), shown
  as a status indicator next to the field — we never echo the actual
  value back to the buffer.
SECRET non-nil masks the editable field with bullets.
SKIP-ELIGIBLE non-nil adds a [ ] Skip checkbox to the right; it
  drives whether the row writes `my/api-skipped-sentinel' on save."
  (widget-insert (format "  %-22s " label))
  ...
  (when skip-eligible
    (let ((cb (widget-create 'checkbox
                             (and current-value
                                  (string= current-value my/api-skipped-sentinel)))))
      (push (cons key cb) my/-config-form-skip-checkboxes))
    (widget-insert " Skip  "))
  (widget-insert (my/-config-form-status current-value))
  (widget-insert "\n"))
...
Bound to the [Save] button. Caches the form's BW email + master into
Keychain FIRST so `my/bw-unlock' uses those instead of re-prompting,
then unlocks (if needed), then dual-writes the rest.

On success exits `recursive-edit' (if any) and kills the form buffer
so the caller (`my/bootstrap-resolve-data-dir-now') can continue."
  (interactive)
  (let ((changes (my/-config-form-collect))
        (saved-anything nil))
    (cond
     ((cl-every (lambda (kv) (eq (plist-get (cdr kv) :value) :unchanged)) changes)
      (message "Setup form: nothing to save (all rows unchanged)."))
     (t
      ;; Step 1: cache form-entered BW email + master to Keychain
      ;; BEFORE attempting unlock, so `my/bw-unlock' takes the
      ;; Keychain-cached path instead of opening its own minibuffer
      ;; prompts. This lets the form act as the single point of entry
      ;; for first-launch credentials.
      (let* ((bw-email-row  (alist-get 'bw-email  changes))
             (bw-master-row (alist-get 'bw-master changes))
             (new-email     (and bw-email-row  (stringp (plist-get bw-email-row  :value))
                                 (plist-get bw-email-row  :value)))
             (new-master    (and bw-master-row (stringp (plist-get bw-master-row :value))
        ...
        (when (and new-master (not (string-empty-p new-master)))
          (my/keychain-set my/keychain-service new-master
                           my/keychain-account-bw-master)))
      ;; Step 2: ensure BW is unlocked. Now uses cached Keychain values
      ;; (just written in step 1) when present, else prompts.
      (unless my/bw-session
        (unless (my/bw-unlock)
          (setq my/-config-form-result 'cancelled)
          (when (> (recursion-depth) 0) (exit-recursive-edit))
          (user-error "Setup form: BW unlock declined or failed — save aborted")))
      (let ((written 0)
            (skipped 0))
        (dolist (kv changes)
          (let* ((key (car kv))
                 (val (plist-get (cdr kv) :value))
                 (env-key (and (stringp (symbol-name key))
                               (string-prefix-p "api/" (symbol-name key))
                               (substring (symbol-name key) 4))))
            (cond
             ((eq val :unchanged) nil)
             (env-key
              ;; API-key row: dual-write Keychain + BW.
              (let ((write-val (if (eq val :skip)
                                   my/api-skipped-sentinel
                                 val)))
                (my/keychain-set my/keychain-service write-val env-key)
                (my/bw-api-key-set env-key write-val)
                (if (eq val :skip) (cl-incf skipped) (cl-incf written))))
             ;; GH-identity rows handled in batch below.
             (t nil))))
        ;; GH identity rows write as one update.
        (let ((gh-fields '()) gh-user gh-token any-gh)
          (dolist (kv changes)
            (let ((key (car kv))
                  (val (plist-get (cdr kv) :value)))
              (when (and (not (eq val :unchanged))
                         (not (eq val :skip))
                         (memq key '(gh-username gh-token gh-email gh-fullname gh-repo)))
                (setq any-gh t)
                (cond
                 ((eq key 'gh-username) (setq gh-user val))
                 ((eq key 'gh-token)    (setq gh-token val))
                 (t                     (push (cons (pcase key
                                                      ('gh-email    "Email")
                                                      ('gh-fullname "Fullname")
                                                      ('gh-repo     "Repo"))
                                                    val)
                                              gh-fields))))))
          (when any-gh
        ...
        (message "Setup form: saved %d / skipped %d row(s) to BW + Keychain."
                 written skipped))))
    ;; Step 4: bookkeeping. Signal success to the caller (so a wrapping
    ;; `recursive-edit' returns), then close the form buffer.
    (setq my/-config-form-result (if saved-anything 'saved 'unchanged))
    (when (> (recursion-depth) 0) (exit-recursive-edit))
...
fields are missing."
  ...
  (let* ((buf (get-buffer-create "*Emacs Setup*"))
    ...
    (with-current-buffer buf
      ...
      (widget-insert "Leave blank to keep the current value. Tick [Skip] to permanently\n")
      (widget-insert "opt out of an API key (__SKIPPED__ sentinel, never re-prompted).\n\n")
      ...
      (widget-insert "\nGitHub identity (in BW.emacs-credentials)\n")
      (widget-insert     "-----------------------------------------\n")
      (my/-config-form-insert-row
       'gh-username "Username"
       (my/-config-form-current-gh-field item 'username) nil nil)
      (my/-config-form-insert-row
       'gh-token "PAT (repo scope)"
       (my/-config-form-current-gh-field item 'password) t nil)
      (my/-config-form-insert-row
       'gh-email "Commit email"
       (my/-config-form-current-gh-field item 'Email) nil nil)
      (my/-config-form-insert-row
       'gh-fullname "Commit name"
       (my/-config-form-current-gh-field item 'Fullname) nil nil)
      (my/-config-form-insert-row
       'gh-repo "Data repo name"
       (my/-config-form-current-gh-field item 'Repo) nil nil)
