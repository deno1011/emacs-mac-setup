...
;; finalized the data-dir (=`~/.emacs.d/data-dir.el' exists). On a
;; FRESH install, that file isn't there yet — `my/data-dir' is still
;; the default `~/emacs/' from init.el's `defvar', not the BW.Repo-
;; derived path bootstrap will write. Creating dirs now would scatter
;; starter directories under the wrong root. Bootstrap writes
;; data-dir.el after reading BW.Repo; user restarts; second run takes
;; the `(my/gtd-ensure-tree)' branch with the correct path.
...
Konfiguriert in [[file:55-apple-reminders.org][55-apple-reminders.org]] — wird vom Modul-Discovery in [[file:../config.org][config.org]] geladen; der =eq system-type 'darwin'=-Guard sitzt innerhalb des Moduls selbst.
