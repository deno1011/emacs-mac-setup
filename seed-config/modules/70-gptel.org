...
#+begin_src emacs-lisp :tangle yes
;; Self-heal — same pattern as 55-apple-reminders.org. Avoid the
;; "Overwrite previous checkout?" prompt when elpa/ contains the
;; package directory but Emacs's package state lost the registration.
(unless (or (package-installed-p 'gptel-agent-runtime)
            (locate-library "gptel-agent-runtime"))
