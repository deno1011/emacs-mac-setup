...
;;
;; Wrapped in `with-eval-after-load' so `gptel-make-anthropic' etc. are
;; only called once gptel + its provider modules have actually loaded.
;; Without this wrap the calls fire at MODULE LOAD time, before elpaca
;; has built gptel (elpaca-process-queues runs on `after-init-hook',
;; which is AFTER this module finishes loading). Symptom otherwise:
;;
;;   CONFIG LOAD ERROR (70-gptel.org): (void-function gptel-make-anthropic)
