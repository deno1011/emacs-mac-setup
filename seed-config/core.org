...
The doom-themes block is kept around for reference but =:tangle no=, so
=doom-one= is no longer loaded. The active theme is the built-in
=modus-vivendi= — accessibility-grade contrast, dark background, ships
with Emacs 28+ so no package install is needed. Same conflict
resolution as the doom variant: disable any theme that may have been
loaded earlier in the init chain before stacking ours, otherwise the
inheritance cycle (modus-vivendi + doom-one earlier) can lock the UI.
...
;; doom block above. Without this, stacking modus-vivendi on top of another
