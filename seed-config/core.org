...
Active theme is =ef-night= from Protesilaos's =ef-themes= collection —
the theme set featured in the System Crafters / Crafted Emacs ecosystem
alongside =modus-themes=. Dark background, high contrast,
violet/blue accents. The doom-themes and the earlier modus-vivendi
blocks are kept for reference with =:tangle no=, so neither is loaded.

Same conflict-resolution pattern in all three blocks: disable whatever
theme may have been loaded earlier in the init chain BEFORE stacking
ours. Without this, the inheritance cycle (e.g. modus-vivendi loaded
under doom-one) can hang Emacs while resolving face inheritance.
...
#+begin_src emacs-lisp :tangle no
;; modus-vivendi (built-in) — accessibility-grade contrast, kept as
;; a fallback option. Re-enable by flipping :tangle no above to yes
;; and :tangle no on the ef-themes block below.
(mapc #'disable-theme custom-enabled-themes)
...
(use-package ef-themes
  :demand t
  :config
  ;; Disable any theme an earlier init step may have loaded — same fix as the
  ;; doom block above. Without this, stacking ef-night on top of another
  ;; loaded theme can cause Emacs to hang while resolving face inheritance.
  (mapc #'disable-theme custom-enabled-themes)
