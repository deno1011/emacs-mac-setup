...
loaded directly. install.sh's `6c. Vendor async-tasks' step writes
one such .el into this directory on every install.")
...
module are logged but do not abort the overall config load.
  ...
  .el  source → byte-compile + load
  .elc current → load directly (~10 ms), no tangle / compile"
