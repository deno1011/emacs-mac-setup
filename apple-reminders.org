...
app.lists().forEach(function(l){
  var items=[];
  l.reminders.whose({completed:false})().forEach(function(r){
    var d=r.dueDate();
    items.push({id:r.id(),title:r.name(),notes:r.body()||'',
                due:d?d.toISOString().slice(0,10):null,
                priority:r.priority(),flagged:r.flagged()});
  });
  out.push({list:l.name(),items:items});
});
JSON.stringify(out);"
  "JXA script returning all open Reminders as JSON.")
...
(defun my/apple-reminders-dashboard ()
  "Open *Apple Reminders* dashboard. Renders from cache instantly, then refreshes."
  (interactive)
...
(defun my/apple-reminders--update-then-refresh (script)
  "Run JXA update SCRIPT async, then trigger a dashboard refresh."
  (message "Apple Reminders: updating…")
  (my/apple-reminders--jxa-async script
    (lambda (_) (my/apple-reminders-dashboard-refresh))))

(defun my/apple-reminders-dashboard-complete ()
  "Complete the reminder at point."
  (interactive)
