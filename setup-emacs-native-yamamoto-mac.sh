...
if brew list | grep -q "emacs-plus"; then
  echo "  HINWEIS: emacs-plus ist ebenfalls installiert."
...
if brew list emacs-mac@30exp &>/dev/null 2>&1; then
  ...
  echo "  emacs-mac@30exp bereits installiert: $EMACS_VER"
  echo "  Zum Deinstallieren: ~/uninstall-emacs-native-yamamoto-mac.sh"
fi
...
if brew list emacs-mac@30exp &>/dev/null 2>&1; then
  skip "Emacs (emacs-mac@30exp)"
else
  echo "==> Installing Emacs (emacs-mac@30exp)..."
...
EMACS_APP=$(find /opt/homebrew/Cellar/emacs-mac@30exp/ -name "Emacs.app" -maxdepth 3 2>/dev/null | head -1)
if [ -n "$EMACS_APP" ]; then
  rm -rf "/Applications/Emacs (Yamamoto).app"
  cp -r "$EMACS_APP" "/Applications/Emacs (Yamamoto).app"
  echo "==> Emacs (Yamamoto).app → /Applications"
fi

EMACS_BIN=$(find /opt/homebrew/Cellar/emacs-mac@30exp*/bin/emacs -maxdepth 2 2>/dev/null | head -1)
...
echo ""
echo "======================================================================"
echo "Native Emacs (Yamamoto) setup complete!"
echo "======================================================================"
echo ""
echo "Start Emacs:  open \"/Applications/Emacs (Yamamoto).app\""
echo ""
if [ -n "$GH_USER" ]; then
