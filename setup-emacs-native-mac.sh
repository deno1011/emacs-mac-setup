...
EMACS_APP=$(find /opt/homebrew/Cellar/ -path "*${_EMACS_PKG}*" -name "Emacs.app" -maxdepth 5 2>/dev/null | head -1)
if [ -n "$EMACS_APP" ]; then
...
EMACS_BIN=$(find /opt/homebrew/Cellar/ -path "*${_EMACS_PKG}*/bin/emacs" 2>/dev/null | head -1)
