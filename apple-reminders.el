...
(defun my/apple-reminders--agenda-tag-ids ()
  "Stamp reminder-id text property on each agenda line from reminders.org.
Uses marker position first; falls back to title search when marker is stale."
  (let* ((sync-file (expand-file-name my/apple-reminders-sync-file))
          ...
          (org-map-entries
           (lambda ()
             (when-let (id (org-entry-get nil "REMINDER_ID"))
               (puthash (string-trim (org-get-heading t t t t)) id title-id)))
           nil nil))
        (save-excursion
          (goto-char (point-min))
          (while (not (eobp))
              ...
              (when (and m (buffer-live-p (marker-buffer m))
                ...
                (let ((id (or
                           ...
                           (let ((txt (get-text-property (point) 'txt)))
                             (when txt
                               (let (found)
                                 (maphash (lambda (title id)
                                            (when (and (not found) (> (length title) 3)
                                                       (string-match-p (regexp-quote title) txt))
                                              (setq found id)))
                                          title-id)
                                 found))))))
                  (when id
...
(defun my/apple-reminders--agenda-mouse-jump (e same-window)
  "Navigate to reminder at mouse event E position, reading reminder-id directly
from the event's buffer/position rather than relying on mouse-set-point side effects."
  (let* ((posn (event-start e))
         (win  (posn-window posn))
         (pt   (posn-point posn)))
    (when (and (windowp win) (numberp pt))
      (let ((id (with-current-buffer (window-buffer win)
                  (get-text-property pt 'reminder-id))))
        (select-window win)
        (if (not id)
            (if same-window (org-agenda-switch-to) (org-agenda-goto))
          (let ((file (expand-file-name my/apple-reminders-sync-file)))
...
(defun my/apple-reminders--agenda-mouse-same (e)
  (interactive "e") (my/apple-reminders--agenda-mouse-jump e t))
(defun my/apple-reminders--agenda-mouse-other (e)
  (interactive "e") (my/apple-reminders--agenda-mouse-jump e nil))
...
(add-hook 'org-agenda-finalize-hook #'my/apple-reminders--setup-agenda-nav)
