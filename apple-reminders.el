...
DONE/CANCELLED → completed=true. TODO/NEXT/WAITING → completed=false (reopen)."
  (unless my/apple-reminders--syncing
    (let ((id   (org-entry-get nil "REMINDER_ID"))
      ...
      (when (and id list)
        ;; Stamp hash for the new state so save hook / C-c r R can tell who last set it.
       ...
       (let* ((id     (org-entry-get nil "REMINDER_ID"))
         ...
         (cond
          ...
          ((and id (member state '("DONE" "CANCELLED")))
           ;; Hash guard: only push if DONE state is new since last stamped push.
           ;; Prevents save hook from perpetually re-closing reminders Apple reopened.
           (let* ((cur-hash (my/apple-reminders--entry-hash))
                  (stored   (org-entry-get nil "REMINDER_HASH")))
             (unless (equal cur-hash stored)
               (my/apple-reminders--complete-in-apple rlist id)
               (org-set-property "REMINDER_HASH" cur-hash))))
  ...
  (let* ((default-list (my/apple-reminders--default-list))
    ...
    (let ((my/apple-reminders--syncing t))
      (with-current-buffer (find-file-noselect file)
        (let (done-pts new-pts reopen-pts apple-updates changed-positions)
                 ...
                 (let ((apple (gethash id apple-by-id)))
                   (cond
                    ((and (member state '("DONE" "CANCELLED"))
                     ...
                     ;; Hash guard: if DONE hash is current (org/sync set it), org wins.
                     ;; If DONE hash is stale, Apple wins → reopen org.
                     (let* ((cur-hash (my/apple-reminders--entry-hash))
                            (stored   (org-entry-get nil "REMINDER_HASH")))
          ...
          (dolist (upd (nreverse apple-updates))
            (cl-destructuring-bind (m _rlist a-prio o-prio a-due o-due a-flagged o-flagged) upd
              ...
              (setq n-updated (1+ n-updated))
              (set-marker m nil)))
          ;; Apply Apple-reopen: DONE in org, open in Apple, stale hash → mark org TODO
        ...
        ;; Stamp REMINDER_HASH for all synced entries (open and done) so save hook
        ;; can tell whether state changed since this sync.
        (org-map-entries
         (lambda ()
