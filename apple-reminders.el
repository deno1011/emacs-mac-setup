...
(defun my/apple-reminders--agenda-mouse-jump (e same-window)
  "Navigate to item at mouse event E. Positions point explicitly before reading
text properties, so it behaves identically to the keyboard TAB/RET handlers."
  (let* ((posn (event-start e))
    ...
    (when (and (windowp win) (numberp pt))
      (select-window win)
      (goto-char pt)
      (my/apple-reminders--agenda-jump same-window))))
