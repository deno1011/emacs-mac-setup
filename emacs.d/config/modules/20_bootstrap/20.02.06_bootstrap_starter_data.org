...
Layer-2 domain operation that seeds missing starter Org and EAR data after
the data repository exists. It copies templates from the distro-managed
=emacs.d/config/starter-data/org/= and =emacs.d/config/starter-data/ear/=
directories into the user-owned data repository.
