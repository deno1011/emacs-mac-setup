...
flag. The orchestrator's reaction depends on it:
  ...
  - The initial data-folder resolution returns =:skip= when no
    =GitHubRepo= credential exists. Personal data setup is deferred;
    data-dependent steps skip until the user runs =M-x my/credential-set=
    and =M-x my/bootstrap=. This keeps a fresh installation usable
    before the user chooses a data repository.
...
The =:not-resolved= sentinel prevents paths from silently falling back
under =~/.emacs.d/=. Every feature module checks
=my/bootstrap-ready-p= before using personal data paths, so Emacs can
start before a data repository is configured. The bootstrap leaves a
setup hint in =*Messages*= and the user can finish the setup later.
...
=my/data-dir-resolve= replaces it with the real path after personal
setup. Until then, data-dependent steps skip and feature modules stay
disabled until the user resolves the path.
...
** Visible first-run setup page

When no private data repository has been selected yet, the bootstrap can
continue safely, but a one-line message is easily buried by package startup
output. Show the user a persistent setup page on the first graphical frame.
