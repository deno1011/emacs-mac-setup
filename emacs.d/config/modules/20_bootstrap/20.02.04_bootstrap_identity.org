#+TITLE: Bootstrap Layer 2 — github identity domain operation
...
Layer-2 domain operation that ensures the GitHub identity stored
in Keychain is reflected in two places the rest of the system
relies on:

  1. Global git config (=user.name=, =user.email=) — so commits
     attribute correctly without prompting.
  2. gh CLI authentication — so private repo operations from
     Emacs's magit / shell-out paths do not block on credential
     prompts.

This is the OPT step #3 in the orchestrator: failures surface in
the final Bootstrap message and in the *Warnings* buffer, but
they do NOT halt the bootstrap. A Mac without gh installed, or
with a revoked token, still boots fine; the user just learns
they need to run a shell command.
...
** =my/identity--ensure-git-config= — sync one git config key
...
** =my/identity--ensure-gh-auth= — sync gh CLI auth from Keychain
...
** =my/identity-ensure-loaded= — orchestrator step #3 (OPT)
...
** =my/identity-credential-descriptors= — descriptors for the unified form
