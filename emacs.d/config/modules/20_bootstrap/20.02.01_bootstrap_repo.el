;;; 20.02.01_bootstrap_repo.el --- Bootstrap Layer 2 repo / data-dir -*- lexical-binding: t -*-
;;
...
absolute data-folder path, and assigns it to `my/data-dir'. A
missing or empty entry defers personal-data setup and returns
`:skip', leaving the base Emacs configuration usable. The caller
surfaces the setup hint.

Idempotent: returns `:skip' when already resolved to the same path,
and `:done' when it assigns a new path."
  (let ((repo (my/keychain-get my/data-dir-resolve--keychain-service
                               my/data-dir-resolve--keychain-account)))
    (cond
     ((or (null repo) (string-empty-p repo))
      :skip)
     (t
...
;;; 20.02.01_bootstrap_repo.el ends here
