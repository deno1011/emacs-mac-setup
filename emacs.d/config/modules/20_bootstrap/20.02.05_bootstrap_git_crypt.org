#+TITLE: Bootstrap Layer 2 — git-crypt unlock domain operation
...
Layer-2 domain operation that decrypts =my/data-dir= when the cloned
repository is git-crypt-enabled. Step #3 in the orchestrator,
required when the repo uses git-crypt — without unlock the data
files (org, wiki, whatever the user committed) sit on disk as
encrypted blobs and Org-mode shows binary garbage.

Decision table:

| State                                                | Result                            |
|------------------------------------------------------+-----------------------------------|
| my/data-dir not resolved                             | (:error "depends on step 1")      |
| folder is not a git repo                             | (:error "depends on step 2")      |
| repo does not use git-crypt                          | :skip                             |
| repo uses git-crypt and is already unlocked          | :skip                             |
| repo uses git-crypt but git-crypt binary missing     | (:error "install git-crypt")      |
| repo uses git-crypt but no key in Keychain           | (:error "store the key, retry")   |
| repo uses git-crypt, key present, unlock fails        | (:error "git-crypt's stderr")     |
| repo uses git-crypt, key present, unlock succeeds     | :done                              |

Per [[file:BOOTSTRAP.md][BOOTSTRAP.md]] §6.4 the (:error …) payload
of every failure case carries the literal `security
add-generic-password' command the user can paste into Terminal.

The Keychain account name is `GitCryptKey:<repo>' where `<repo>'
is the last path segment of `my/data-dir' — same scheme the
legacy bootstrap used so users can migrate from the previous
implementation without re-importing their key.
...
** =my/git-crypt-ensure-unlocked= — orchestrator step
...
** =my/git-crypt-credential-descriptors= — for the unified form

The GitCryptKey account name is repo-specific, so the descriptor
list is only meaningful once `my/data-dir' has resolved to a real
path. While the bootstrap is in skeleton state the function returns
nil and the credential menu skips this slot.
