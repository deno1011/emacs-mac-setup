...
(defun my/bootstrap--schedule-setup-hint (frame)
  "Schedule the setup page after FRAME has completed client initialization."
  (when (and (not my/bootstrap--setup-hint-shown-p)
             (not (my/bootstrap-ready-p))
             (display-graphic-p frame))
    (run-at-time 0.75 nil #'my/bootstrap--show-setup-hint frame)))

(add-hook 'after-make-frame-functions #'my/bootstrap--schedule-setup-hint)
