...
secondary-number/ToS decision, so it is *not* auto-provisioned.
=69_external_provisioning.org= creates the private checkout, installs Node
dependencies, and seeds a mode-0600 =.env= template without touching it later.
The remaining personal QR step is =M-x my/whatsapp-adapter-link=, which renders
it inside Emacs; =M-x my/doctor= reports the state at every point.
...
On a normal setup, wait for the external-provisioning queue to finish, then
edit the seeded =.env= only for personal values:
...
The external-provisioning queue installs =signal-cli=, creates the private
adapter checkout, installs its Node dependencies, and seeds a safe =.env=
template.  Then complete the inherently personal steps inside Emacs (these
handle the Node PATH for you, so prefer them over a raw terminal):
