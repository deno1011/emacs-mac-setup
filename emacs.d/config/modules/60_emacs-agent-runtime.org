...
The private repository also owns =ear-overrides/=. It mirrors the complete EAR
package by relative path: =ear-overrides/lisp/modules/runtime/ear-runtime.el=
shadows =<EAR>/lisp/modules/runtime/ear-runtime.el=, while a missing private
path falls back to Core.
This setup installs private Lisp directories before the first EAR =require=,
then EAR applies the same rule to declarations and path-aware tools. Use
=ear-config/= for additive declarations, =ear-extensions/lisp/= for additive
features, and =ear-overrides/= only when an existing Core source path must be
replaced. Restart Emacs after changing an already loaded Lisp feature.
...
The setup exposes one reusable workspace entry point and one user-owned
repository root.  The root contains a declaration overlay with the standard
=agents/shared/state= shape plus =ear-extensions/lisp/= for private executable
features and =ear-extensions/adapters/= for deployed adapters with personal
configuration or auth state.  Reusable adapter checkouts live below
=<EAR checkout>/external/adapters/=.  These roots are created idempotently and
their locations are controlled centrally by
=my/emacs-agent-runtime-core-adapters-directory= and
=my/emacs-agent-runtime-private-adapters-directory=.  Declarations load after
the workspace, while Core remains readable but immutable to normal EAR tools:

#+begin_src emacs-lisp :tangle no
(setq ear-config-directory
      (expand-file-name "declarations/workspace/" my/emacs-agent-runtime-dir)
      ear-config-extra-directories
      (list (expand-file-name "ear-config/" my/data-dir))
      ear-config-authoring-directory
      (expand-file-name "ear-config/" my/data-dir)
      ear-config-private-directory my/data-dir
      ear-config-extension-directories
      (list (expand-file-name "ear-extensions/lisp/" my/data-dir)))
#+end_src

Private declaration files may replace shipped ids through normal later-layer
registration.  Private Lisp should use unique feature names and be referenced
from =ear-config/shared/tool-implementations/*.eld=.  The setup passes the same
private root, declarations, and extension load paths to process workers, so a
tool does not disappear merely because an agent runs out of process.

The path is bound before gptel can load EAR transitively.  Once EAR is present,
the setup performs one explicit declaration reconciliation, creates the default
runtime, and starts every registered job marked =:auto-start t=.  Repeated setup
calls are idempotent, while a new Emacs process repeats the reconciliation and
therefore restores those jobs after a daemon or machine restart.

=M-x my/emacs-agent-runtime-scheduler-health= reports how many registered
auto-start jobs are running and names any missing job.
...
The normal setup path is =69_external_provisioning.org=: its =standard=
profile installs the reviewed QMD executable after Node is ready.  This command
remains a focused repair/install override when QMD alone needs attention:
