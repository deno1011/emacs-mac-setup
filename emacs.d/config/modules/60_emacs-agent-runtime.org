...
For the current development phase this setup loads EAR from
=~/emacs-agent-runtime=. That avoids the old GAR package recipe and keeps the
runtime independent from one AI client. Later this can become an elpaca package
install for other machines, but the interface should stay the same.
