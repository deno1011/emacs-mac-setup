...
** Visual Tables And Modern Org Styling
#+begin_src emacs-lisp
...
The Layer-2 bootstrap now creates a compact GTD starter set directly
inside the user's data repository when files are missing:

- =data/org/inbox.org= for capture
- =data/org/gtd.org= for the GTD system, manual, examples, projects,
  next actions, waiting-for, someday/maybe, and review lists
- =data/org/calendar.org= for dated commitments
- =data/org/archive.org= for archived completed/cancelled items

This Org module only keeps the directory root present at runtime. It
does not seed or overwrite files; =my/starter-data-ensure= owns that
idempotent first-install behavior during bootstrap.
...
~d2~ is optional for EAR dashboard/system maps and other generated diagrams.
It is not part of Org Babel here, but belongs to the same diagram-rendering
toolchain. Provision it explicitly with
...
** Optional Diagram Renderer Provisioning

These renderer CLIs are shared by Org/Babel inline graphics and EAR's graphical
dashboard. The setup installer stays lean; this explicit command is the
idempotent path when a user wants the full diagram surface.
