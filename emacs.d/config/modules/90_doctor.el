...
(defun my/doctor--whatsapp-registered-auth-p (auth-dir)
  "Return non-nil when AUTH-DIR contains fully registered Baileys credentials."
...
(defun my/doctor--whatsapp-usable-auth-p (auth-dir)
  "Return non-nil when AUTH-DIR contains credentials that can reconnect.

Baileys v7 can leave WhatsApp Business auth with `registered=false' while the
linked device is already connected and syncing.  A saved self identity plus an
account-sync timestamp is therefore treated as usable auth."
  (let ((creds (expand-file-name "creds.json" auth-dir)))
    (when (file-readable-p creds)
      (with-temp-buffer
        (insert-file-contents creds)
...
(defun my/doctor--whatsapp-status-file (channel)
  "Return the adapter runtime status file for CHANNEL."
  (expand-file-name
   (format "messenger-bridge/status/%s.json" channel)
   user-emacs-directory))

(defun my/doctor--whatsapp-status (channel)
  "Read WhatsApp adapter runtime status for CHANNEL as an alist, or nil."
  (let ((file (my/doctor--whatsapp-status-file channel)))
...
(defun my/doctor--whatsapp-status-open-p (channel)
  "Return non-nil when CHANNEL's runtime status says it is connected."
  (let ((status (my/doctor--whatsapp-status channel)))
    (and (equal (alist-get 'state status) "open")
         (alist-get 'connected status)
         (alist-get 'selfJid status))))
...
(my/doctor-define-check "Claude subscription token (EAR claude -p)"
  (let ((stored   (and (fboundp 'my/api-key-fetch)
                       (my/api-key-fetch "ANTHROPIC_SUBSCRIPTION_API_KEY")))
        (exported (getenv "CLAUDE_CODE_OAUTH_TOKEN")))
    (cond
     ((and (stringp stored) (not (string-empty-p stored))
           (stringp exported) (not (string-empty-p exported)))
      (list :status :ok
            :detail "token in Keychain and CLAUDE_CODE_OAUTH_TOKEN exported to claude -p"))
     ((and (stringp stored) (not (string-empty-p stored)))
      (list :status :warn
            :detail "token stored but CLAUDE_CODE_OAUTH_TOKEN not exported this session"
            :fix "M-x my/emacs-agent-runtime-set-subscription-token (re-applies), or reload 60_emacs-agent-runtime"))
     (t
      (list :status :warn
            :detail "no subscription token set — EAR claude -p may bill pay-per-use and fail with 'Credit balance is too low'"
            :fix "Run `! claude setup-token`, then M-x my/emacs-agent-runtime-set-subscription-token")))))
...
(my/doctor-define-check "WhatsApp adapter (optional messaging channel)"
  (let* ((dir my/whatsapp-adapter-dir)
         ...
         (web-wa-dns
          (when (file-executable-p "/usr/bin/dscacheutil")
            (with-temp-buffer
              (call-process "/usr/bin/dscacheutil" nil t nil
                            "-q" "host" "-a" "name" "web.whatsapp.com")
              (buffer-string)))))
    (cond
     ((not (eq system-type 'darwin))
      (list :status :ok :detail "not macOS — skipped"))
     ...
     ((not (and web-wa-dns (string-match-p "\\S-" web-wa-dns)))
      (list :status :warn
            ...
            :fix "Fix DNS/network first (System Settings > Network > Wi-Fi > Details > DNS, or disable VPN/DNS filter), then rerun M-x my/whatsapp-adapter-link"))
     ((not (file-exists-p creds))
      (list :status :warn
     ...
     ((my/doctor--whatsapp-status-open-p "whatsapp")
      (list :status :ok
            :detail (format "running and connected as %s"
                            (alist-get 'selfJid
                                       (my/doctor--whatsapp-status "whatsapp")))))
     ((not (my/doctor--whatsapp-usable-auth-p auth))
      (list :status :warn
            :detail "adapter auth exists but does not contain a usable linked identity"
            :fix "M-x my/whatsapp-adapter-link — it will archive incomplete auth and show a fresh QR"))
     (t
      (list :status :ok
            :detail "linked (usable auth present) — M-x my/whatsapp-adapter-install-service for persistence")))))

(my/doctor-define-check "WhatsApp Business adapter (secondary fitness coach)"
  (let* ((dir my/whatsapp-adapter-dir)
         (nm (expand-file-name "node_modules" dir))
         (auth (expand-file-name "auth-business" dir))
         (creds (expand-file-name "creds.json" auth))
         (business-env (expand-file-name ".env.whatsapp-business.example" dir))
         (business-plist
          (expand-file-name "launchd/com.deno1011.whatsapp-business-bridge.plist"
                            dir))
         (web-wa-dns
          (when (file-executable-p "/usr/bin/dscacheutil")
            (with-temp-buffer
    ...
    (cond
     ...
     ((not (and web-wa-dns (string-match-p "\\S-" web-wa-dns)))
      (list :status :warn
            ...
            :fix "Fix DNS/network first, then rerun M-x my/whatsapp-business-adapter-link"))
     ((not (file-exists-p creds))
      (list :status :warn
            :detail "secondary Business adapter ready but NOT linked (no auth-business/creds.json)"
            :fix "M-x my/whatsapp-business-adapter-link — scan the QR in WhatsApp Business > Linked Devices"))
     ((my/doctor--whatsapp-status-open-p "whatsapp-business")
      (list :status :ok
            :detail (format "running and connected as %s"
                            (alist-get 'selfJid
                                       (my/doctor--whatsapp-status "whatsapp-business")))))
     ((not (my/doctor--whatsapp-usable-auth-p auth))
      (list :status :warn
            :detail "secondary Business auth exists but does not contain a usable linked identity"
            :fix "M-x my/whatsapp-business-adapter-link — it will archive incomplete auth-business and show a fresh QR"))
     (t
      (list :status :ok
...
(my/doctor-define-check "Org/EAR optional diagram renderers"
  (let* ((renderers (if (and (boundp 'my/org-diagram-renderers)
                             (listp my/org-diagram-renderers))
         ...
         (present nil)
         (missing nil)
         (install (when (fboundp 'my/org-diagram-renderers-install-command)
                    (my/org-diagram-renderers-install-command))))
    (dolist (entry renderers)
      (let ((exe (car-safe entry))
            (formula (cdr-safe entry)))
        (if (and (stringp exe) (executable-find exe))
            (push (format "%s=%s" exe (executable-find exe)) present)
          (push (format "%s (%s)" exe formula) missing))))
    (if missing
        (list :status :warn
              :detail (format "missing optional renderer(s): %s"
                              (string-join (nreverse missing) ", "))
              :fix (or install
                       "M-x my/org-diagram-renderers-install"))
      (list :status :ok
            :detail (string-join (nreverse present) ", ")))))
