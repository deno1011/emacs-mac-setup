...
Keep the left Option key as Emacs Meta for the normal =M-x= workflow.  On a
German macOS keyboard, reserve the right Option key for native character input
instead; =right Option-L= then produces =@= for email addresses and EAR's
inline selectors.  Configure both the NS and Mac-port variable names so the
setting survives a switch between Emacs builds.
...
Vertico uses compatibility functions from the current =compat= package.
Load the Emacs 31 compatibility layer explicitly on older Emacs versions.
This is necessary even when =compat= itself is already present: bytecode
compiled under Emacs 31 can otherwise omit the runtime load of =compat-31=,
leaving functions such as =set-local= undefined after a restart.
