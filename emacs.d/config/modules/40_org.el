...
(use-package org-modern
  ...
  :config
  ;; Enable org-modern everywhere.  The :hook alone did not reliably activate
  ;; it (org buffers rendered plain); the global mode does, and also styles
...
;; Data-dir.el gate (RESTORED): on a first-launch this form
;; Only run when bootstrap has converged — otherwise `my/data-dir'
;; is the `:not-resolved' sentinel and `expand-file-name' would
;; type-error. `(my/bootstrap-ready-p)' is the new design's gate;
;; the legacy `(file-exists-p data-dir.el)' check it replaced is
;; gone because the new bootstrap does not write data-dir.el.
(when (my/bootstrap-ready-p)
  (my/gtd-ensure-tree))
...
(when (my/bootstrap-ready-p)
  (use-package org-roam
    ...
    :config
    ;; `org-roam-db-autosync-mode' opens the DB file at this path —
    ;; create the directory if a freshly-cloned data repo doesn't
    ;; carry the wiki/emacs/ tree yet. Idempotent.
