;;; 70_wiki.el --- LLM-maintained Emacs wiki -*- lexical-binding: t; -*-
...
Initialised to nil at defvar time because `my/data-dir' may still
be the `:not-resolved' sentinel when this module loads (bootstrap
not yet converged). The `setq' below populates it when bootstrap
is ready; otherwise it stays nil and downstream wiki helpers
short-circuit on `(stringp my/wiki-base-dir)' checks.")

(when (my/bootstrap-ready-p)
  (setq my/wiki-base-dir (expand-file-name "wiki/emacs/" my/data-dir)))
...
;; Only run when bootstrap has converged — `my/wiki-base-dir' is nil
;; until the (when (my/bootstrap-ready-p) (setq …)) above populates
;; it. `(my/bootstrap-ready-p)' is the new design's gate; the legacy
;; `(file-exists-p data-dir.el)' check it replaced is gone because
;; the new bootstrap does not write data-dir.el.
(when (my/bootstrap-ready-p)
  (my/wiki-ensure-tree))
