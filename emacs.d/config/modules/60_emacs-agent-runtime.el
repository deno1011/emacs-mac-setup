...
(defcustom my/emacs-agent-runtime-external-runtime-directory
  (expand-file-name "ear/external/" user-emacs-directory)
  "User-local runtime root for provisioned external adapter dependencies.

Adapter source checkouts remain in their Core or private repositories.  This
directory contains only reproducible machine-local runtime state such as Python
virtual environments.  It is deliberately outside both Git worktrees and is
safe to recreate on a fresh machine."
  :type 'directory
  :group 'emacs-agent-runtime)
...
(defun my/emacs-agent-runtime-external-runtime-root-directory ()
  "Return the machine-local root for provisioned external dependencies."
  (file-name-as-directory
   (expand-file-name my/emacs-agent-runtime-external-runtime-directory)))

(defun my/emacs-agent-runtime-external-python (name)
  "Return the managed Python interpreter for external runtime NAME.

Before the matching virtual environment has been provisioned, fall back to a
normal `python3' executable so Doctor and the owning adapter can report a
useful prerequisite state instead of failing during configuration load."
  (let ((candidate
         (expand-file-name
          (format "python/%s/bin/python" name)
          (my/emacs-agent-runtime-external-runtime-root-directory))))
    (if (file-executable-p candidate)
        candidate
      (or (executable-find "python3") "/usr/bin/python3"))))
...
;; The launchd service selects the model; this list advertises compatible
;; OpenAI-style model identifiers to gptel and EAR.
(let ((backend (gptel-make-openai "MLX"
  ...
  (setq my/gptel-backends
        (append my/gptel-backends
                `(("Llama 3.2 3B (MLX)" ,backend . mlx-community/Llama-3.2-3B-Instruct-4bit)
