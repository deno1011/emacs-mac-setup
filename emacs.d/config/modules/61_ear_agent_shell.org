#+TITLE: EAR Agent Shell Integration
#+STARTUP: overview
#+PROPERTY: header-args:emacs-lisp :tangle yes :lexical t

Loaded by: [[file:../config.org][config.org]] ← [[file:~/.emacs.d/init.el][init.el]]

This optional integration connects the MIT-licensed Emacs Agent Runtime to the
GPL-3.0-licensed Agent Shell package through the Agent Client Protocol.  It is
separate from EAR Core and from the general runtime loader so the dependency,
license boundary, and replacement point remain explicit.
