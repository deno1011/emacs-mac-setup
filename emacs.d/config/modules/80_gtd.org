...
* Apple GTD Provisioning

So bekommt ein neuer Nutzer die macOS-Umgebung (Reminders-Listen, „Org"-
Kalender) automatisch — **ohne** wissen zu müssen, dass es sie gibt. Alles
läuft über die **gekapselte Paket-API** (org-apple-reminders /
org-apple-calendar), nie über Ad-hoc-JXA hier; ein Backend-Wechsel bleibt damit
paket-intern. Idempotent (legt nichts doppelt an). Die „Habits"-Liste wird
mit angelegt (leer); Habit-Inhalte/Reminder bleiben deine Wahl. Wiederkehrende
Review-Rituale werden NICHT in Apple angelegt — die Review-Kadenz lebt
org-seitig als Tickler-Items (sichtbar für Agent/Coach); die Paket-API
=ensure-recurring= bleibt für manuelles Anlegen in Emacs verfügbar.
