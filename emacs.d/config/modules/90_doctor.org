...
#+begin_src emacs-lisp :tangle yes
(my/doctor-define-check "iCloud CalDAV password"
  (cond
   ((not (eq system-type 'darwin))
    (list :status :ok :detail "not macOS — skipped"))
   ((not (and (boundp 'org-caldav-url) (stringp org-caldav-url)
              (string-match-p "icloud" org-caldav-url)))
    (list :status :ok :detail "iCloud calendar sync not configured — skipped"))
   ((and (fboundp 'my/keychain-get-internet)
         (my/keychain-get-internet "caldav.icloud.com"))
    (list :status :ok :detail "app-specific password present in Keychain"))
   (t
    (list :status :warn
          :detail "org-caldav configured but no app-password in Keychain"
          :fix "M-x my/caldav-set-password"))))
#+end_src
