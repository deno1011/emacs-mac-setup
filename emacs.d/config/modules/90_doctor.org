...
The optional adapters are converged by =69_external_provisioning.org=, not by
=install.sh=.  This Doctor entry distinguishes an intentionally disabled
profile, a queue that is still working, a deferred private repository, and an
actual provisioning failure.  It never attempts a login, QR pairing, or model
download.
