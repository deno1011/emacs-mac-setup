...
4. Sets the standard gptel default to Gemini 2.0 Flash. If
   =GEMINI_API_KEY= is present, requests work immediately; if the key is
   still missing, the selected default remains Gemini and Emacs tells the
   user to add the key. Ollama remains an optional local backend and no
   longer steals the default merely because it is installed.
...
(when-let* ((entry (assoc "Gemini 2.0 Flash" my/gptel-backends))
  ...
  (message "gptel default: Gemini 2.0 Flash%s"
           (if (getenv "GEMINI_API_KEY")
               ""
             " (set GEMINI_API_KEY in secrets.el before sending requests)")))
#+end_src
