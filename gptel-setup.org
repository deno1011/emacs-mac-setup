...
3. Registers Anthropic, OpenAI, LM Studio, MLX, and Ollama backends with
   the model lists previously embedded in the package.
4. Sets a preliminary default model (LM Studio Ministral, matching the
   prior package behavior) and then asks the runtime to switch to local
   Ollama. Cloud models remain registered for manual switching, but the
   runtime/planner default stays local.
...
;; -- Google Gemini (commented until a key is available) -------------
