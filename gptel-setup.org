...
User-side gptel directive for the Emacs-knowledge LLM Wiki at
=~/emacs/wiki/emacs/=. Pattern: Karpathy's LLM Wiki — a persistent,
LLM-maintained knowledge base of interlinked org-roam pages.

The directive is built dynamically from =WIKI.org= (the schema) and
=index.org= (the catalog), so edits to either propagate after
=M-x my/wiki-maintainer-refresh=. The directive is *not* auto-selected
by =gptel-agent-runtime-directive-for-current-runtime=; switch to it
manually via gptel's directive menu.
