...
(defun gptel-agent-runtime-use-default-local-model ()
  "Select the configured default local Ollama model when available."
  (interactive)
  (gptel-agent-runtime-start-ollama-if-needed)
  (when my/gptel-ollama-backend
    (let ((model gptel-agent-runtime-default-local-model))
      ...
      (my/gptel-sync-tools)
      (message "gptel default local model: %s (%s)"
               gptel-agent-runtime-default-local-model-label model))))
