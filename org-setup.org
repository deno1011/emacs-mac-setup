...
(defvar my/-agenda-path-excludes
  '("/\\.git/" "/archive/" "/elpa/" "/#[^/]+#\\'" "/\\.#")
  "Path regexps universally excluded from the agenda scan
(structural filesystem noise — not user content).")

(defun my/-agenda-files-scanned ()
  "Return all .org files under `my/data-dir' except those matching
a path in `my/-agenda-path-excludes'."
  (let ((root (file-name-as-directory my/data-dir))
        result)
