...
(defvar my/gtd-bootstrap-files
  '("inbox.org"
    ...
    "gtd/someday.org"
    "gtd/reference.org")
  "Generic GTD starter files seeded by `my/gtd-bootstrap'. Paths
relative to my/data-dir's data/org/. Each is fetched from
emacs-mac-setup/stable/gtd-templates/<path> on first install if
absent locally.")

(defvar my/gtd-bootstrap-url
  "https://raw.githubusercontent.com/deno1011/emacs-mac-setup/stable/gtd-templates/"
  "Base URL for fetching GTD starter templates on first install.")

(defvar my/-gtd-bootstrap-marker
  (expand-file-name ".bootstrap-completed"
                    (expand-file-name "data/org/" my/data-dir))
  "Marker file. Once present, the GTD layer's auto-fetch is locked.
See the marker file content for the full explanation.")

(defun my/gtd-bootstrap ()
  "Create ~/emacs/data/org/{gtd,archive}/ and seed missing GTD starter
files from templates. Idempotent — only creates what is absent.
Auto-runs on package load; can also be invoked as M-x my/gtd-bootstrap.
Returns t if anything was created, nil otherwise.

Once `my/-gtd-bootstrap-marker' exists, this is a complete no-op —
the user owns the GTD layer."
  (interactive)
  (if (file-exists-p my/-gtd-bootstrap-marker)
      nil
    (my/-gtd-bootstrap-do)))

(defun my/-gtd-bootstrap-do ()
  "Actual GTD bootstrap work. Internal — call `my/gtd-bootstrap'."
  (let ((created nil)
        (org-root (expand-file-name "data/org/" my/data-dir)))
    ;; 1. Directory tree (data/org/ + gtd/ + archive/)
    (dolist (sub '("" "gtd" "archive"))
      (let ((d (expand-file-name sub org-root)))
        (unless (file-directory-p d)
          (make-directory d t)
          (setq created t))))
    ;; 2. Seed missing GTD files from templates.
    ;; Resolution order per file: local sibling → adjacent mac-setup
    ;; checkout → /tmp/emacs-mac-setup → fetch from public stable.
    (dolist (rel my/gtd-bootstrap-files)
      (let ((dest (expand-file-name rel org-root)))
        (unless (file-exists-p dest)
          ...
          (let ((src (cl-find-if
                      #'file-readable-p
                      (list
                       (expand-file-name (concat "gtd-templates/" rel) my/data-dir)
                       (expand-file-name (concat "../emacs-mac-setup/gtd-templates/" rel)
                                         my/data-dir)
                       (expand-file-name (concat "/tmp/emacs-mac-setup/gtd-templates/" rel))))))
            (cond
             (src
              (copy-file src dest)
              (setq created t))
             (t
              (condition-case err
                  (progn
                    (require 'url)
                    (url-copy-file (concat my/gtd-bootstrap-url rel) dest t)
                    (setq created t))
                (error
                 (message "gtd-bootstrap: could not fetch %s: %s"
                          rel (error-message-string err))))))))))
    ;; Drop the marker so future runs skip the entire bootstrap. The
    ;; marker is a verbose self-documenting file written by
    ;; my/-bootstrap-write-marker (defined in init.el).
    (when (fboundp 'my/-bootstrap-write-marker)
      (my/-bootstrap-write-marker org-root "gtd"))
    (when (and created (called-interactively-p 'any))
      (message "GTD bootstrapped at %s" org-root))
    created))

;; Auto-bootstrap on package load. No-op once files exist.
(my/gtd-bootstrap)
#+end_src
...
(condition-case _err
  ...
  (error (message "bh/org-agenda-to-appt: skipped at load -- %S" _err)))
(appt-activate t)
...
(use-package org-roam
  ...
  :custom
  (org-roam-directory (expand-file-name "~/emacs/wiki/emacs"))
  (org-roam-db-location (expand-file-name "~/emacs/wiki/emacs/.org-roam.db"))
...
Konfiguriert in [[file:org-reminders-setup.org][org-reminders-setup.org]] — wird von [[file:config.org][config.org]] nur auf macOS geladen.
