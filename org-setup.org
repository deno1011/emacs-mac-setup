...
- *websocket-bridge*: Abhängigkeit von org-reminders, nicht auf MELPA —
  wird automatisch via =package-vc-install= installiert
- *Python-Package*: =pip3 install websocket-bridge-python= —
  wird beim ersten Start automatisch installiert (via =my/pip-install-and-log= aus core.org)
- *Emacs 29+*: für =package-vc-install=

** Installation & Konfiguration

#+begin_src emacs-lisp :tangle yes
;;; Apple Reminders via org-reminders
;;; Abhängigkeiten: reminders-cli (brew) + websocket-bridge (GitHub) + websocket-bridge-python (pip)

;; reminders-cli prüfen
(unless (executable-find "reminders")
  (message "org-reminders: reminders-cli fehlt — brew install keith/formulae/reminders-cli"))

;; Python-Abhängigkeit (websocket-bridge-python)
(when (fboundp 'my/pip-install-and-log)
  (my/pip-install-and-log "websocket-bridge-python"))
...
;; org-reminders installieren (findet websocket-bridge jetzt im load-path)
...
(use-package org-reminders
  :ensure nil
  :config
  ;; Org-Datei für den bidirektionalen Sync
  (setq org-reminders-reminders-to-org-file
        (expand-file-name "~/org/reminders.org"))
  ;; In org-agenda-files einbinden
